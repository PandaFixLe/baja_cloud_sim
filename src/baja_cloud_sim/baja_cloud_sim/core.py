...
class StanleyControllerConfig(ControllerConfig):
    ...
    lookahead_time_s: float = 0.15  # predictive lookahead: planner + actuation latency (s)
...
def augment_path_for_cte(path: Sequence[Point]) -> List[Dict[str, float]]:
    """Convert Sequence[Point] to dicts with synthesized math-frame yaw and cumulative s.

    The yaw at each point is the bearing of the outgoing segment (or the previous
    bearing for the last point).  ``s`` is the cumulative polyline arc-length from
    the first point.  Used by stanley_path_control, lqr_path_control, and tests.
    """
    ...
    for index, point in enumerate(path):
        ...
        if index > 0:
            s_cum += math.hypot(point[0] - path[index - 1][0],
                                point[1] - path[index - 1][1])
        augmented.append({"x": point[0], "y": point[1], "yaw": yaw, "s": s_cum})
    return augmented
...
def stanley_path_control(
    ...
    actual_v = max(state.prev_speed, 0.5)
    v_safe = max(actual_v, cfg.min_speed_for_stanley)
    ...
    if cfg.adaptive_steering:
        ...
        max_steer = math.radians(cfg.max_steering_deg) / scale
    else:
...
def _lookahead_s(
    ...
    """Integrate along the speed profile until accumulated time ≥ lookahead_time_s.

    Returns the arc-length offset to add to the start position.  On curves the
    speed profile drops, so the lookahead distance naturally shortens —
    the car doesn't "overshoot" into a turn it can't handle at current speed.
    """
    ...
    for i in range(start_idx, len(pts) - 1):
        ...
        if ds <= 1e-9:
            continue
        v_avg = (pts[i].speed_limit + pts[i + 1].speed_limit) * 0.5
        if v_avg < 0.1:
            continue
        dt = ds / v_avg
        if t_accum + dt >= lookahead_time_s:
            remaining = lookahead_time_s - t_accum
            s_accum += v_avg * remaining
            break
        t_accum += dt
        s_accum += ds
    return s_accum
...
def _s_to_index(pts, s_target: float) -> int:
    """Return index of the first trajectory point with s >= s_target.

    pts[].s is monotonic (generated by uniform arc-length resampling).
    Linear scan is O(n) on ~50 points — fast enough at 20 Hz.
    """
    if not pts:
        return 0
