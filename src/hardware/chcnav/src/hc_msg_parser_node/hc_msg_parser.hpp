...
public:
    std::shared_ptr<rclcpp::Node> nh;         // 节点handle
     ...
     * */
    hc__msg_parser_node(unsigned int hz, hc__device_connector *device_connector, std::shared_ptr<rclcpp::Node> nh, std::shared_ptr<rclcpp::Node> private_nh, string node_name)
    {
        if (NULL != nh && NULL != private_nh && NULL != device_connector)
        {
            ...
            // 订阅写话题
            this->write_suber = this->private_nh->create_subscription<msg_interfaces::msg::Int8Array>("write", 20, std::bind(&hc__msg_parser_node::node_write, this, std::placeholders::_1));
    ...
    void node_start(void)
    {
        ...
        //spinner.spin();
        // 创建ROS 2回调队列
        auto callback_group = nh->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
        // 创建ROS 2异步执行器
        auto executor = std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
        // 启动异步执行器
        executor->add_node(nh);

        // 在独立线程中 spin executor, 让订阅回调 (如 /write 接收 RTCM) 能被真正派发;
        // 主线程继续跑串口解析的 while 循环。
        std::thread spin_thread([executor]() { executor->spin(); });
        spin_thread.detach();

        usleep(300 * 1000); // 等待异步执行器启动完成

        rclcpp::Rate loop_rate(this->hz);
        while (rclcpp::ok()) // 此处循环表示直到成功取到一次数据（指通过校验，hc cgi 自定义协议或者 nmea 协议）后才 break 该层循环进入 sleep
        {
            while (1)
            {
     ...
     * @brief 设备写入信息话题回调函数
     * */
    void node_write(const msg_interfaces::msg::Int8Array::ConstPtr msg)
    {
...
int msg_recv_callback(hc__single_msg_t single_msg, hc__msg_parser_node *node)
{
    ...
    switch (single_msg.msg_type)
    {
        case HC__MSG_NMEA:
            ...
            node->nmea_msg.header.stamp = node->nh->now();
        ...
        case HC__MSG_CGI_HEADER:
            ...
            node->hc_msg.header.stamp = node->nh->now();
