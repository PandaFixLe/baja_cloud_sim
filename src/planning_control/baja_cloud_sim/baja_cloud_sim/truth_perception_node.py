...
class TruthPerceptionNode(Node):
    ...
    def _publish_truth(self) -> None:
        ...
        # Ground-truth road boundaries (from scenario file).  Published on
        # /road_boundary_markers so the planner always has a corridor even when
        # the perception group's road_analyzer fails to detect lane edges from
